LWDKExec(function(){
    ...
    FormCreateAction("dados_produto", function(data){
        ...
        $.post("{myurl}", {cadprod: data}, function(success){
            if(success===true){
                successRequest(("{acao}" === "cadastrado" ? function(){window.top.location.href="/produto/listar/";}:function(){LWDKLoadPage(LWDKLocal, LWDKInitFunction.exec)}), "O produto foi {acao} com sucesso!");
            } else {
